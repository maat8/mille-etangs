...
class ParcoursRepository extends EntityRepository
{
    public function findAllVtt()
    {
        return $this->getEntityManager()
            ->createQuery('SELECT p FROM MilleEtangsRandonneesBundle:Parcours p WHERE p.duree_vtt > 0 AND p.publie = true ORDER BY p.nom ASC')
            ->getResult();
    }
    ...
    {
        return $this->getEntityManager()
            ->createQuery('SELECT p FROM MilleEtangsRandonneesBundle:Parcours p WHERE p.duree_marche > 0 AND p.publie = true ORDER BY p.nom ASC')
            ->getResult();
    }
